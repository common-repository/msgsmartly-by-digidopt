...
/*
 *
 * Plugin Name: MsgSmartly by Digidopt
 * Plugin URI: https://msgsmartly.com/
 * Description: Connect with your customers on one single platfrom, Best Omnichannel Platfrom. Connect Facebook, Instagram, Twitter, Whatsapp business API, Telegram, Slack.
 * Version: 1.0.0 
 * Author: Digidopt
 * Author URI: https://digidopt.net/
 * © 2021-2022 Digidopt. All rights reserved.
 *
 */

function sbcloud_set_admin_menu() {
    add_submenu_page('options-general.php', 'MsgSmartly by Digidopt', 'MsgSmartly  by Digidopt', 'administrator', 'msgsmartly', 'sbcloud_admin');
}

function sbcloud_enqueue_admin() {
    if (key_exists('page', $_GET) && $_GET['page'] == 'msgsmartly') {
...
function sbcloud_enqueue() {
    ...
    if ($settings['synch-wp-users']) {
        ...
        if ($current_user) {
            ...
            $inline_code .= 'var SB_DEFAULT_USER = { first_name: "' . esc_html($current_user->user_firstname) . '", last_name: "' . esc_html($current_user->user_lastname) . '", email: "' . esc_html($current_user->user_email) . '", profile_image: "' . esc_html($profile_image) . '", password: "' . esc_html($current_user->user_pass) . '", extra: { "wp-id": [' . esc_html($current_user->ID) . ', "WordPress ID"] }};';
        }
    }
    ...
    wp_enqueue_script('chat-init', 'https://chat.msgsmartly.com/account/js/init.js?id=' . esc_html($settings['chat-id']) . $language, ['jquery'], '1.0');
...
function sbcloud_admin() { 
    if (isset($_POST['sbcloud_submit'])) {
        $chat_id = sanitize_text_field( $_POST['sbcloud-chat-id'] );
...
<div class="wrap">
    <h1>MsgSmartly by Digidopt</h1>
	
	<div >
    <img src="/wp-content/plugins/msgsmartly-by-digidopt/assets/logo.png" alt="MsgSmartly Logo" width="300" height="100">
</div>
            ...
            <tbody>
                <tr valign="top">
                    ...
                    <td>
                        ...
                        <br />
                        <p class="description">Enter the embed code or the ID attribute. Get it from <a target="_blank" href="https://chat.msgsmartly.com/account/?tab=installation">here</a>. Pricing <a target="_blank" href="https://msgsmartly.com/pricing">here</a>.</p>
                ...
                <tr valign="top">
                    ...
                    <td>
                        ...
                        <p class="description">
                            ...
                            This setting requires a WordPress Multisite installation and MsgSmartly departments with the same IDs of the WordPress websites.
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                ...
                <tr valign="top">
                    ...
                    <td>
                        ...
                        <br />
                        <p class="description">
                          Sync logged in WordPress users with MsgSmartly.
                        </p>
                    </td>
                </tr>
                <tr valign="top">
...
add_shortcode('msg-tickets', 'sbcloud_tickets_shortcode');
add_shortcode('msg-articles', 'sbcloud_articles_shortcode');
